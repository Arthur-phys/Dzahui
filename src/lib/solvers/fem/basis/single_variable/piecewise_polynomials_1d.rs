...
impl PiecewiseFirstDegreePolynomial {
    ...
    pub fn from_values(coefficients: Vec<f64>, independent_terms: Vec<f64>, interval_breakpoints: Vec<f64>) -> Result<Self, Error> {

        if independent_terms.len() != interval_breakpoints.len() + 1 || independent_terms.len() != coefficients.len() {
            return Err(Error::PieceWiseDims);
        }

        let polynomials = coefficients.into_iter().zip(independent_terms).map(|(coef, i_term)| -> FirstDegreePolynomial {
    ...
    pub fn from_constants(independent_terms: Vec<f64>, interval_breakpoints: Vec<f64>) -> Result<Self,Error> {
        
        if independent_terms.len() != interval_breakpoints.len() + 1 {
            return Err(Error::PieceWiseDims);
        }

        let polynomials = independent_terms.into_iter().map(|i_term| -> FirstDegreePolynomial {
            FirstDegreePolynomial::new(0_f64, i_term)
        }).collect();
...
impl Function1D for PiecewiseFirstDegreePolynomial {
    ...
    fn evaluate(&self, x: f64) -> f64 {
        ...
        let val = self.interval_breakpoints.iter().enumerate().find_map(|(i,breakpoint)| -> Option<f64> {
            if x < *breakpoint {
...
impl Differentiable1D<PiecewiseFirstDegreePolynomial> for PiecewiseFirstDegreePolynomial {
    ...
    fn differentiate(&self) -> PiecewiseFirstDegreePolynomial {
        ...
        let diff_polynomials = self.polynomials.iter().map(|pol| -> FirstDegreePolynomial {
            pol.differentiate()
        }).collect();
